...
def rent_ratio ():
    ...
    city = entry.get()
    state = entry2.get()
    full_state = entry3.get()
    # The above statements get the values to search for based on user input for the app
    
    # Working with rent data.
    # Need to split Location column into City and State seperate columns
    sep = rent['Location'].str.split(", ", n = 1, expand = True)
    rent['City_'] = sep[0]
    rent['State_'] = sep[1]
    
    # Select bedroom size of 3 for rented properties to experiment with.
    # Assumed that 3 bdrm size is comparable to single family home.
    rent_c = rent[(rent['City_'] == city) & (rent['Bedroom_Size'] == '3br')&(rent['State']==state)]
    
    # Collapse the monthly dates that were columns to rows
    rent_cm = pd.melt(rent_c)
    
    # Dropped unused rows to leave a dataframe with only the prices for each month.
    # Drop newly split colummns, Location, Location_Type, State, Bedroom_Size
    rent_cm.drop([0,1,2,3,78,79], inplace = True)
    
    # Replace underscores used in date column to hyphen, 
    # and drop the string "Price" from the values
    rent_cm['var2'] = rent_cm['variable'].str.replace("_","-")
    for c in rent_cm['var2']:
        rent_cm['var4'] = rent_cm['var2'].apply(lambda x : x[6:] 
                                                  if x.startswith("P") 
                                                  else x)
    ...
    #Update the data type of the column to dates and the rent value column to float.
    rent_cm['var4'] = pd.to_datetime(rent_cm['var4'])
    rent_cm['value'] = rent_cm['value'].astype(float)
    
    rent_cm.columns = ['City_Rent_Price', 'Date_']
    
    # Process data set for Historical home value
    
    # Need to select city based on city name and state, to select unique cities
    ...
    city_comb['Rent_Ratio'] = city_comb.City_Home_Price / ((city_comb.City_Rent_Price) * 12)
    ...
    # Plot how a cities rent ratio behaves with its home sale counts
    date1 = city_.Date_.astype('O')
    fig,ax = plt.subplots(figsize=(10,5))
    ax.plot(date1, city_.Rent_Ratio, color = 'purple', label="Rent Ratio")
    ax.set(ylabel = 'Rent Ratio', title=str(city)+","+str(state)+ " City home sales and Rent ratio")
    ax.legend(loc=4)
    #ax.set(title= city)
    ax2 = ax.twinx()
    ...
    ax2.legend(loc=2)
    #plt.title(str(city)+","+str(state), fontsize=14)
    plt.show()
    ...
    ## Provide plot of the now stationary version to model the sales 
    plt.plot(df_sales['Date_'], (df_sales['diff']), color ='b')
    plt.title("Stationary Home Sales " + str(city) +","+ str(state))
    plt.show()
    plt.plot(df_ratio['Date_'], (df_ratio['diff_r']), color ='r')
    plt.title ("Stationary Rent Ratio " + str(city) +","+ str(state))
    plt.show()
    ...
    ## Begin model with using the lags on regression for the difference in sales as y to the forecasted lags
    import statsmodels.formula.api as smf
    
    # Define the regression formula
    model_s = smf.ols(formula='diff ~ lag_1+lag_2+lag_3+lag_4+lag_5+lag_6+lag_7\
    +lag_8+lag_9+lag_10+lag_11+lag_12+lag_13+lag_14+lag_15+lag_16\
    +lag_17+lag_18+lag_19+lag_20+lag_21', data=df_s)
    ...
    df_s_model = df_s.drop(['City_Sales','Date_','City_Rent_Price','City_Home_Price','Rent_Ratio'],axis=1)
    #split train and test set
    train_dfs, test_dfs = df_s_model[0:-12].values, df_s_model[-12:].values
    
    
    df_r_model = df_r.drop(['City_Sales','Date_','City_Rent_Price','City_Home_Price','diff_r'],axis=1)
    #split train and test set
    ...
    model_3 = Sequential()
    model_3.add(LSTM(4, batch_input_shape=(1, X_train3.shape[1], X_train3.shape[2]), stateful=True))
    model_3.add(Dense(1))
    model_3.add(Dropout(0.2))
    model_3.compile(loss='mean_squared_error', optimizer='adam')
    model_3.fit(X_train3, y_train3, epochs= epochs1, batch_size=1, verbose=1, shuffle=False)
    
    
    ## Setup for history reference for loss plot
    his = model_3.fit(X_train3, y_train3, epochs = epochs1, batch_size=1, verbose=1, shuffle=False, \
                      validation_data=(X_test3, y_test3), callbacks=[EarlyStopping(monitor='val_loss', patience=10)])
    ...
    model2.fit(X_train2, y_train2, epochs=epochs1, batch_size=1, verbose=1, shuffle=False)
    ## Setup for history reference for loss plot
    hist = model2.fit(X_train2, y_train2, epochs=epochs1, batch_size=1, verbose=1, shuffle=False, \
                      validation_data=(X_test2, y_test2), callbacks=[EarlyStopping(monitor='val_loss', patience=10)])
    model2.summary()
    ...
    for index in range(0,len(y_pred2)):
        ...
        pred_test_set2.append(numpy.concatenate([y_pred2[index],X_test2[index]],axis=1))
    ...
    pred_test_dfs = pred_test_dfs.reshape(pred_test_dfs.shape[0], pred_test_dfs.shape[2])
    ...
    for index in range(0,len(pred_test_dfs_inverted)):
        result_dict_3 = {}
        result_dict_3['pred_value'] = int(pred_test_dfs_inverted[index][0] + act_sales_3[index])
    ...
    for index2 in range(0,len(pred_test_set_inverted2)):
        result_dict2 = {}
        result_dict2['pred_ratio'] = (pred_test_set_inverted2[index2][0] + act_ratio[index2])
    ...
    plt.figure(figsize=(10,4))
    plt.plot(date3, new_df3.City_Sales, color ='b', label = 'Historical Home Sales')
    plt.plot(date3, new_df3.pred_value, color ='gold', label = 'Predicted Home Sales', ls ='--') 
    plt.legend()
    plt.title(str(city)+","+str(state) + " Home Sales Model performance with Actual performance for last 12 months")
    plt.show() 
    
    plt.figure(figsize=(10,4))
    plt.plot(date2, df_ratio.Rent_Ratio, color ='r', label = 'Historical Rent Ratio')
    plt.plot(date2, new_df2.pred_ratio/2, color ='gold', label = 'Predicted Rent Ratio', ls ='--') 
    plt.legend()
    plt.title(str(city)+","+str(state)+ " Rent ratio Model performance with Actual performance for last 12 months")
    plt.show()
    ...
    df_predict3 = pd.DataFrame(scaler_t.inverse_transform(ypp),\
                           index=future_dates_3[-13:].index, columns =['Prediction'])
    
    df_predict4 = pd.DataFrame(scaler_tt.inverse_transform(yp2_l),\
                           index=future_dates_4[-13:].index, columns =['Ratio_Prediction'])
    ...
    plt.plot(df_cs.Date_, (df_cs.Prediction)*-2.78, color ='b', label = 'Forecasted Home Sales', ls ='--') 
    plt.legend()
    plt.title(str(city)+","+str(state)+ " Home sales historical and forecasted")
    plt.show() 
    
    plt.figure(figsize=(10,4))
    plt.plot(df_rr.Date_, df_rr.Rent_Ratio, color ='r',label='Historical Rent Ratio')
    plt.plot(df_rr.Date_, (df_rr.Ratio_Prediction)*1.075, color ='r', ls ='--',label='Forecasted Rent Ratio')
    plt.legend()
    plt.title(str(city)+","+str(state)+ " Rent Ratio historical and forecasted")
    plt.show() 
    

    fig,ax = plt.subplots(figsize=(10,4))
    ax.plot(df_cs.Date_, (df_cs.Prediction)*-2.78, color = 'blue', ls='--', label='Homes Sales Forecast')
    ax2 = ax.twinx()
    ax2.plot(df_cs.Date_,(df_rr.Ratio_Prediction)*1.075, color ='red', ls='--', label ='Rent Ratio Forecast')
    #ax.set_ylabel('Rent Ratio')
    ax.legend(loc = 4)
    ax.set(ylabel = 'Home sales forecast')
    ax.set(title = str(city)+","+str(state) + " Forecasted city home sales and rent ratio")
    ...
    ## Obtain list of all cities historical data for the main data frame of 
    ## city home sales, rent data, rent ratio, and home values
    s = pd.merge(mort,ts,on='RegionID',how='inner')
    rn = rent.copy()
    sp = rn['Location'].str.split(", ", n = 1, expand = True)
    rn['RegionName_x'] = sp[0]
    rn['State_'] = sp[1]
    
    rn1 = rn.loc[rn['Bedroom_Size'] == '3br']
    sms = pd.merge(s,rn1,how='inner',on='RegionName_x')
    #print(sms)
    ...
    plt.subplot(1,2,2)
    sns.boxplot(x="quarter", y="Rent_Ratio", data=dfa)
    plt.xlabel('quarter')
    plt.title('Box plot of Quarterly Rent Ratio: '+str(city)+","+str(state))
    sns.despine(left=True)
    plt.tight_layout()
    plt.show()
    
    
    # Histogram of rent ratio and city sales 
    
    plt.figure(figsize=(14,6))
    plt.subplot(1,2,1)
    dfa['City_Sales'].hist(bins=50)
    plt.title('City Sales Distribution ' + str(city)+","+str(state))
    plt.subplot(1,2,2)
    stats.probplot(dfa['City_Sales'], plot=plt)
    #dfa.describe().T
    plt.show()
    
    plt.figure(figsize=(14,6))
    plt.subplot(1,2,1)
    dfa['Rent_Ratio'].hist(bins=50)
    plt.title('Rent Ratio Distribution '+ str(city)+","+str(state))
    plt.subplot(1,2,2)
    stats.probplot(dfa['Rent_Ratio'], plot=plt)
    dfa.describe().T
    plt.show()
    
    stat, p = stats.normaltest(dfa.City_Sales)
    ...
    sns.distplot(dfa.City_Sales, label = 'ayeee')
    print( 'Kurtosis of normal distribution: {}'.format(stats.kurtosis(dfa.City_Sales))+" "+str(city)+","+str(state))
    print( 'Skewness of normal distribution: {}'.format(stats.skew(dfa.City_Sales))+" "+str(city)+","+str(state))
    ...
    print('Train Mean Absolute Error:'+" "+str(city)+","+str(state), mean_absolute_error(Y_train[0], train_predict[:,0]))
    print('Train Root Mean Squared Error:'+" "+str(city)+","+str(state),numpy.sqrt(mean_squared_error(Y_train[0], train_predict[:,0])))
    print('Test Mean Absolute Error:'+" "+str(city)+","+str(state), mean_absolute_error(Y_test[0], test_predict[:,0]))
    print('Test Root Mean Squared Error:'+" "+str(city)+","+str(state),numpy.sqrt(mean_squared_error(Y_test[0], test_predict[:,0])))
    



#Create Labels for Application - This is where user inputs Data strings for city, state


lbl_val = tk.Label(master = window, text = "City")
lbl_val.pack()
entry = tk.Entry()
entry.pack()

lbl_val2 = tk.Label(master = window, text = "State Abbr")
lbl_val2.pack()
entry2 = tk.Entry()
entry2.pack()

lbl_val3 = tk.Label(text = "State Full Name")
lbl_val3.pack()
entry3 = tk.Entry()
entry3.pack()
...
button = tk.Button(text = "click when done", command = rent_ratio)
button.pack()
tk.Button(window, text="Clear", command=window.destroy and clear).pack()
